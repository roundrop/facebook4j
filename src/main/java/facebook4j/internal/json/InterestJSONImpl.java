 ...
 */
/*package*/ final class InterestJSONImpl extends CategorizedEntityJSONImpl implements Interest, java.io.Serializable {
    ...
    static ResponseList<Interest> createInterestList(HttpResponse res, Configuration conf) throws FacebookException {
        try {
            ...
            for (int i = 0; i < size; i++) {
                Interest interest = new InterestJSONImpl(list.getJSONObject(i));
                interests.add(interest);
            }
            if (conf.isJSONStoreEnabled()) {
                DataObjectFactoryUtil.registerJSONObject(interests, json);
            }
            return interests;
