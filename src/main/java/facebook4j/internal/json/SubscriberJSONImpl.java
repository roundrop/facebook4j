 ...
 */
/*package*/ final class SubscriberJSONImpl extends IdNameEntityJSONImpl implements Subscriber, java.io.Serializable {
    ...
    static ResponseList<Subscriber> createSubscriberList(HttpResponse res, Configuration conf) throws FacebookException {
        try {
            ...
            for (int i = 0; i < size; i++) {
                Subscriber subscriber = new SubscriberJSONImpl(list.getJSONObject(i));
                subscribers.add(subscriber);
            }
            if (conf.isJSONStoreEnabled()) {
                DataObjectFactoryUtil.registerJSONObject(subscribers, json);
            }
            return subscribers;
