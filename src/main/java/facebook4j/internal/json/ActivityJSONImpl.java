 ...
 */
/*package*/ final class ActivityJSONImpl extends CategorizedEntityJSONImpl implements Activity, java.io.Serializable {
    ...
    static ResponseList<Activity> createActivityList(HttpResponse res, Configuration conf) throws FacebookException {
        try {
            ...
            for (int i = 0; i < size; i++) {
                Activity activity = new ActivityJSONImpl(list.getJSONObject(i));
                activities.add(activity);
            }
            if (conf.isJSONStoreEnabled()) {
                DataObjectFactoryUtil.registerJSONObject(activities, json);
            }
            return activities;
