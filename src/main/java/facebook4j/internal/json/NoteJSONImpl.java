 ...
 */
/*package*/ final class NoteJSONImpl implements Note, java.io.Serializable {
    ...
    static ResponseList<Note> createNoteList(HttpResponse res, Configuration conf) throws FacebookException {
        try {
            ...
            for (int i = 0; i < size; i++) {
                Note note = new NoteJSONImpl(list.getJSONObject(i));
                notes.add(note);
            }
            if (conf.isJSONStoreEnabled()) {
                DataObjectFactoryUtil.registerJSONObject(notes, json);
            }
            return notes;
        } catch (JSONException jsone) {
